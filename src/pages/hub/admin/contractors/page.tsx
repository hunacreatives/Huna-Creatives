...
export default function ContractorsPage() {
  ...
  return (
    ...
    >
                ...
                <div key={c.id} className="grid grid-cols-[40px_minmax(0,1fr)_auto] sm:grid-cols-[40px_minmax(0,1fr)_150px_110px_90px_56px_76px] items-center gap-4 px-5 py-4 hover:bg-gray-50/60 transition-colors cursor-pointer group"
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {c.status === 'pending' && (
                        <span className="text-[10px] px-1.5 py-0.5 bg-orange-100 text-orange-600 rounded-full font-medium flex-shrink-0">not invited yet</span>
                      )}
      ...
      {/* Toast notifications */}
      <div className="fixed bottom-[max(1.5rem,env(safe-area-inset-bottom,0px)+5.5rem)] lg:bottom-6 right-6 z-50 flex flex-col gap-2 pointer-events-none">
        {toasts.map(t => (
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 space-y-4">
            ...
            <button
              ...
              className="w-full bg-[#FF6B35] text-white text-sm py-2 rounded-lg hover:bg-[#e55a27] transition-colors"
            >
              Done
            </button>
          </div>
