...
export default function ContractorCredentialsPage() {
  ...
  const [approvedData, setApprovedData] = useState<Record<string, ApprovedData>>({});
  ...
  const fetchData = async () => {
    ...
    // Get all credentials (non-sensitive fields only)
    ...
    const credList = (creds as CredentialCatalog[]) ?? [];
    const reqList = (reqs as MyRequest[]) ?? [];
    ...
    // For approved credentials, fetch full data
    const approvedIds = reqList.filter((r) => r.status === 'approved').map((r) => r.credential_id);
    if (approvedIds.length > 0) {
      const { data: full } = await supabase
        .from('hub_credentials')
        .select('id, password, additional_info')
        .in('id', approvedIds);
      const map: Record<string, ApprovedData> = {};
      (full ?? []).forEach((c: any) => {
        map[c.id] = { password: c.password, additional_info: c.additional_info };
      });
      setApprovedData(map);
    } else {
      setApprovedData({});
  ...
  const openRequestModal = (cred: CredentialCatalog) => {
      ...
      <div className="p-6 space-y-5">
        ...
        <div className="flex items-start justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">Request access to client platform credentials</p>
          </div>
        </div>
                  ...
                  {isExpanded && (
                    <div className="border-t border-gray-50 divide-y divide-gray-50">
                      {clientCreds.map((cred) => {
                        ...
                        const fullData = isApproved ? approvedData[cred.id] : null;
                          ...
                          <div key={cred.id} className="px-5 py-4 hover:bg-gray-50/50 transition-colors">
                            <div className="flex items-start justify-between gap-3">
                              <div className="flex-1 min-w-0 space-y-1.5">
                                ...
                                {/* Password — only shown if approved */}
                                ...
                                {/* Additional info (approved only) */}
                                {isApproved && fullData?.additional_info && (
                                  <p className="text-xs text-gray-400">{fullData.additional_info}</p>
                                )}
          ...
          <div className="bg-white rounded-2xl w-full max-w-md">
            <div className="flex items-center justify-between p-5 border-b border-gray-100">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-0.5">
                  {requestModal.platform} — {requestModal.client_name}
                </p>
              </div>
            ...
            <div className="flex gap-2 p-5 pt-0">
              ...
              <button
                ...
                className="flex-1 py-2.5 text-sm bg-[#FF6B35] text-white rounded-lg hover:bg-[#e55a24] disabled:opacity-40 cursor-pointer transition-colors whitespace-nowrap"
              >
